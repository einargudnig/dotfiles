...
Capture the session as a breadcrumb (via `/memento`), update MEMORY.md with pending work pointers, then prompt the user to `/clear`.
...
1. Invoke the `/memento` skill to capture session knowledge as a breadcrumb (decisions, discoveries, patterns)
...
4. If there are pending items, update the project's `MEMORY.md`:
   ...
   - Do NOT write session summaries here — those belong in the breadcrumb
...
| Content | Destination |
|---------|-------------|
| Session narrative, decisions, discoveries | Breadcrumb (via `/memento`) |
| Pending/unfinished work pointers | MEMORY.md |
| User preferences, stable config | MEMORY.md (if not already there) |

## What to skip

- Session narrative in MEMORY.md (the breadcrumb handles that)
